...
function drawStartScreen() {
  ...
  textSize(18);
  text("Try to catch as many circles with your nose as you can before time runs out.", width / 2, height / 2);
  fill(0, 255, 0);
...
function mousePressed() {
  if (gameState === "start" && mouseX > width / 2 - 50 && mouseX < width / 2 + 50 && mouseY > height / 2 + 20 && mouseY < height / 2 + 60) {
    gameState = "play";
    startGame();
  } else if (gameState === "end" && mouseX > width / 2 - 50 && mouseX < width / 2 + 50 && mouseY > height / 2 + 50 && mouseY < height / 2 + 90) {
    resetGame();
    gameState = "play";
  }
}
